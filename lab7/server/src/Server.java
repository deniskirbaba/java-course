 ...
 * @version 1.0
 * Server class that connects clients to the server and performs receiving and sending messages from clients
 */
public class Server extends Thread
{
    ...
    /** Constructor */
    public Server(int port, Stack<Movie> movies)
    {
        this.manager = new Manager(movies);
        this.port = port;
    }
    ...
    /**
     * Method to extract command from Object command, execute and send at the client
     */
    public void applyCommand(Object command, SocketChannel client) throws IOException
    {
        ...
        switch (commandName.toLowerCase())
        {
            case "info":
                ...
                info.initialize(this.manager.getCollectionTypeClass() ,this.manager.getInitializationDate(),
                        this.manager.getNumberOfObjectsInCollection());
                send(client, info);
                break;
            case "add":
                Add add = (Add) command;
