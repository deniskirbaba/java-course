...
public class Movie implements Serializable, Comparable<Movie>
{
    ...
    /** Field for storage the name of the user who created this object */
    private String user;
     ...
     * Constructor for making a movie object
     */
    public Movie(int id, String name, Coordinates coordinates, String creationDate, long oscarsCount, Integer goldenPalmCount, MovieGenre genre, MpaaRating mpaaRating, Person screenwriter, String user)
    {
        this.id = id;
        this.creationDate = creationDate;
        this.name = name;
    ...
    /**
     * Method to add id to the id list
     */
    public static void addToIdList(int id)
    {
        Movie.idList.add(id);
    }
    ...
    {
        try {
            if ((movie.coordinates.getX() < 415) && (movie.coordinates.getY() < 212) && (movie.oscarsCount > 0) &&
                    (movie.id > 0) && (movie.name != null) && (movie.goldenPalmCount > 0) && (movie.genre != null) &&
                    (movie.mpaaRating != null) && (movie.screenwriter.getName() != null) &&
                    (movie.screenwriter.getHeight() > 0) && (movie.screenwriter.getWeight() > 0) &&
                    (!Movie.idList.contains(movie.id))) {
                String[] time = movie.creationDate.trim().split("\\s+");
                String hourMinute = time[0];
                String dayMonthYear = time[1];
